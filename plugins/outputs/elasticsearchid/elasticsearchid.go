...
func (a *Elasticsearch) Write(metrics []telegraf.Metric) error {
	...
	for _, metric := range metrics {
		...
		var br *elastic.BulkIndexRequest

		if a.IDField == "" {
			br = elastic.NewBulkIndexRequest().Index(indexName).Doc(m)
		} else {
			var fieldList = metric.FieldList()
			var id string

			for _, field := range fieldList {
				if field.Key == a.IDField {
					if str, ok := field.Value.(string); ok {
						id = str
					} else {
						return fmt.Errorf("Elasticsearch id_field value is not a string")
					}
				}
			}

			br = elastic.NewBulkIndexRequest().Id(id).Index(indexName).Doc(m)
		}
...
func init() {
	outputs.Add("elasticsearch-custom-id", func() telegraf.Output {
		return &Elasticsearch{
			Timeout:             duration{Duration: time.Second * 5},
			HealthCheckInterval: duration{Duration: time.Second * 10},
		}
	})
}
